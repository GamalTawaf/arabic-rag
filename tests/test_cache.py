...
async def test_lookup_returns_none_when_the_cache_is_empty(db_session):
    assert await lookup(db_session, QUESTION, unit_vector(0), "bge", 0.95) is None


async def test_store_then_lookup_returns_the_cached_answer(db_session):
    # Arrange
    await store(db_session, QUESTION, unit_vector(0), "bge", ANSWER, CITATIONS)

    # Act
    cached = await lookup(db_session, QUESTION, unit_vector(0), "bge", 0.95)
...
async def test_lookup_misses_when_the_question_is_unrelated(db_session):
    ...
    assert await lookup(db_session, "سؤال مختلف", unit_vector(7), "bge", 0.95) is None
...
async def test_lookup_hits_just_above_the_threshold_and_misses_just_below(db_session):
    ...
    assert await lookup(db_session, QUESTION, near, "bge", 0.94) is not None
    assert await lookup(db_session, QUESTION, near, "bge", 0.96) is None
...
async def test_lookup_never_matches_a_vector_from_another_model(db_session):
    ...
    await store(db_session, QUESTION, unit_vector(0), "e5", ANSWER, CITATIONS)

    assert await lookup(db_session, QUESTION, unit_vector(0), "bge", 0.95) is None
    assert await lookup(db_session, QUESTION, unit_vector(0), "e5", 0.95) is not None
...
async def test_lookup_rejects_a_semantic_match_whose_digits_differ(db_session):
    ...
    await store(
        db_session, "ما مدة الإشعار بعد 3 سنوات؟", unit_vector(0), "bge", ANSWER, []
    )

    cached = await lookup(
        db_session, "ما مدة الإشعار بعد 5 سنوات؟", unit_vector(0), "bge", 0.95
    )
...
async def test_lookup_rejects_a_semantic_match_whose_negation_differs(db_session):
    ...
    await store(db_session, "هل يجوز فصل العامل؟", unit_vector(0), "bge", ANSWER, [])

    cached = await lookup(
        db_session, "هل لا يجوز فصل العامل؟", unit_vector(0), "bge", 0.95
    )
...
async def test_lookup_accepts_a_paraphrase_with_the_same_digits_and_negation(db_session):
    ...
    await store(
        db_session, "كم مدة الإشعار بعد 3 سنوات؟", unit_vector(0), "bge", ANSWER, []
    )

    cached = await lookup(
        db_session, "كم مدة الإنذار بعد 3 سنوات؟", unit_vector(0), "bge", 0.95
    )
...
async def test_the_guard_over_rejects_interrogative_ma(db_session):
    ...
    """
    await store(db_session, "ما مدة الإشعار؟", unit_vector(0), "bge", ANSWER, [])

    assert await lookup(db_session, "كم مدة الإشعار؟", unit_vector(0), "bge", 0.95) is None
...
async def test_a_hit_increments_hits_and_stamps_last_hit_at(db_session):
    ...
    # Act
    await lookup(db_session, QUESTION, unit_vector(0), "bge", 0.95)
    await lookup(db_session, QUESTION, unit_vector(0), "bge", 0.95)
...
async def test_a_guard_rejection_does_not_count_as_a_hit(db_session):
    await store(db_session, "بعد 3 سنوات", unit_vector(0), "bge", ANSWER, [])

    await lookup(db_session, "بعد 5 سنوات", unit_vector(0), "bge", 0.95)
...
async def test_store_normalizes_the_query_it_persists(db_session):
    await store(db_session, "الإجازةُ السَّنوية", unit_vector(0), "bge", ANSWER, [])
...
async def test_store_ignores_an_empty_answer(db_session):
    await store(db_session, QUESTION, unit_vector(0), "bge", "   ", [])
