...
`config/Dockerfile` installs CPU torch and `sentence-transformers` alongside
...
The `config/Dockerfile` comment names the build step that trades image size for cold-start
...
docker build --platform linux/amd64 -f config/Dockerfile -t "$IMAGE" .   # linux/amd64 matters on Apple Silicon
docker push "$IMAGE"
```
